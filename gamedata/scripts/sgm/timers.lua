...
_M.check_and_respawn_community_war = function()
   local par_war = {}
...
--------------------------------------------------------------------------------------------------------
---==================================================================================================---
-- todo переписать под реактивность
local Rx = require("gamedata.scripts.lib.rx")
-- Rx.Observable.fromRange(1, 8)
-- :filter(function(x) return x % 2 == 0 end)
-- :concat(Rx.Observable.of('who do we appreciate'))
-- :map(function(value) return value .. '!' end)
-- :subscribe(give_quick_news)
_M.update = function()
   ...
   if ui_mod_elements.army_timer_active == true then
      ui_mod_elements.army_timer_value = string.format(math.floor(time_global() / 1000)) -
      ui_mod_elements.army_timer_deprive
   else
   ...
   if ui_mod_elements.mp3_plays ~= nil then
      ui_mod_elements.mp3_length_position = string.format(math.floor(time_global() / 1000)) -
      ui_mod_elements.mp3_last_position
   end
   ...
   if check_seconds(10) then
      ...
      elseif check_game_timer("web_money_course_timer") then
         write_mod_param("web_money_course",
   ...
   if check_seconds(5) then
      ...
      elseif check_game_timer("community_war_respawn_timer") then
         _M.check_and_respawn_community_war()
         start_game_timer("community_war_respawn_timer", 30)
      end
   end

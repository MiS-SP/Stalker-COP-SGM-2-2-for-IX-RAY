...
local BlackDay = Class()
function BlackDay:init()
...
function BlackDay:blackday_start()
   if find_in_string(level.get_weather(), "default") then
      ...
      elseif present_night() then
         start_game_timer("blackday_main_timer",
            math.random(get_blackday_frequency(1, "night"), get_blackday_frequency(2, "night")))
...
function BlackDay:blackday_preparate()
   self:blackday_weather_start()
   write_mod_param("blackday_stage", "step_2")
   SendScriptCallback("black_day_began")
end

function BlackDay:blackday_end()
   ...
   self:blackday_birth()
   SendScriptCallback("black_day_end")
end

function BlackDay:blackday_abort()
   write_mod_param("blackday_stage", "step_end")
   start_game_timer("blackday_main_timer", math.random(get_blackday_frequency(1), get_blackday_frequency(2)))
   abort_game_timer("blackday_length_timer")
   disable_info("blackday_is_active")
...
function BlackDay:blackday_safe(blackday_stage)
   if check_blackday_precond() == false and blackday_stage ~= "step_end" then
      write_mod_param("blackday_stage", "step_end")
   end
end

function BlackDay:blackday_weather_start()
...
function BlackDay:blackday_weather_end()
   if level_weathers.get_weather_manager().weather_fx then
      level.stop_weather_fx()
   end
end

function BlackDay:blackday_birth()
...
function BlackDay:show_blackday_info()
   show_hud_message("real=" ..
      get_general_game_time() ..
      "|target_time=" .. check_game_timer("blackday_main_timer", true) .. "|weather=" .. level.get_weather())
end
